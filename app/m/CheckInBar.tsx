 ...
 *  check-in is active. Tapping the info area (left + middle) navigates to
 *  /m with ?openMachine=<id> so CheckInTab can auto-open the sheet for the
 *  active machine. The Check out button stays as its own button so it
...
export function CheckInBar() {
  ...
  return (
    <div
      className="fixed left-0 right-0 z-30 bg-emerald-600 text-white shadow-lg flex items-stretch"
    ...
    >
      <Link
        href={href}
        prefetch={false}
        className="flex-1 flex items-center gap-3 px-4 py-2 active:bg-emerald-700"
      >
      ...
      <button
        type="button"
        onClick={checkOut}
        className="px-4 py-2 my-2 mr-2 ml-1 rounded-md bg-white/15 hover:bg-white/25 active:bg-white/25 text-xs font-semibold"
      >
        Check out
      </button>
    </div>
